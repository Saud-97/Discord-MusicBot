...
const command = new SlashCommand().setName("play").setDescription(
	...
	if (res.loadType === "LOAD_FAILED") {
		...
		return interaction.editReply({
			embeds: [
				new MessageEmbed().setColor("RED").setDescription("There was an error while searching"),
			],
		}).catch(this.warn);
	}
	...
	if (res.loadType === "NO_MATCHES") {
		...
		return interaction.editReply({
			embeds: [
				new MessageEmbed().setColor("RED").setDescription("No results were found"),
			],
		}).catch(this.warn);
	}
	...
	if (res.loadType === "TRACK_LOADED" || res.loadType === "SEARCH_RESULT") {
		...
		let addQueueEmbed = new MessageEmbed().setColor(client.config.embedColor)
			...
			).setURL(res.tracks[0].uri).addField("Added by", `<@${ interaction.user.id }>`, true).addField(
				"Duration",
		...
		return interaction.editReply({ embeds: [addQueueEmbed] }).catch(this.warn);
	}
	...
	if (res.loadType === "PLAYLIST_LOADED") {
		...
		return interaction.editReply({ embeds: [playlistEmbed] }).catch(this.warn);
	}
});
