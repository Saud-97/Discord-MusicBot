...
const command = new SlashCommand().setName("help").setDescription("Shows this list")
	.setRun(async (client, interaction) => {
		...
		const helpEmbed = new MessageEmbed().setColor(client.config.embedColor).setAuthor({
			name: `Commands of ${ client.user.username }`,
			iconURL: client.config.iconURL,
		}).setTimestamp().setFooter({ text: `Page ${ pageNo + 1 } / ${ maxPages }` });
		...
		const getButtons = (pageNo) => {
			return new MessageActionRow().addComponents(
				new MessageButton().setCustomId("help_cmd_but_2_app").setEmoji("◀️").setStyle("PRIMARY")
					.setDisabled(pageNo == 0),
				new MessageButton().setCustomId("help_cmd_but_1_app").setEmoji("▶️").setStyle("PRIMARY")
		...
		collector.on("collect", async (iter) => {
			...
			tempArray.forEach((cmd) => {
				//console.log(cmd);
				helpEmbed.addField(cmd.name, cmd.description).setFooter({ text: `Page ${ pageNo + 1 } / ${ maxPages }` });
			});
			helpEmbed.addField(
				"Credits",
				`Discord Music Bot Version: v${
