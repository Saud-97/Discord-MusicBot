...
const command = new SlashCommand().setName("search").setDescription("Search for a song").addStringOption((option) =>
	...
	try {
		...
		if (res.loadType === "LOAD_FAILED") {
			return interaction.reply({
				embeds: [
					new MessageEmbed().setDescription("An error occured while searching for the song").setColor("RED"),
				],
				ephemeral: true,
			});
		}
	...
	if (res.loadType == "NO_MATCHES") {
		return interaction.reply({
			embeds: [
				new MessageEmbed().setDescription(`No results found for \`${ search }\``).setColor("RED"),
			],
			ephemeral: true,
		});
	} else {
		...
		let choosenTracks = await interaction.editReply({
			embeds: [
				new MessageEmbed().setColor(client.config.embedColor).setDescription(
			...
			if (i.isSelectMenu()) {
				...
				i.editReply({
					...
					embeds: [
						...
						}).setURL(res.tracks[0].uri).setThumbnail(res.tracks[0].displayThumbnail("maxresdefault")).setDescription(
							`[${ trackForPlay?.tracks[0]?.title }](${ trackForPlay?.tracks[0].uri })` ||
							"No Title",
						).addField("Added by", `<@${ interaction.user.id }>`, true).addField(
							"Duration",
