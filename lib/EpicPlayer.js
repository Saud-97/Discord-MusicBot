	...
	(Player) =>
		class extends Player {
			...
			async sendNowplayingMessage(client, disabled = false, track = this.track) {
				...
				const channel = await client.channels.cache.get(this.textChannel);
				
				channel.messages.fetch({ limit: 1 }).then(async (messages) => {
					if (messages.first().id === this.nowPlayingMessage?.id) {
						this.nowPlayingMessage.edit(msg);
					} else {
						let nowPlayingMessage = await channel.send(msg).catch(client.warn);
						this.setNowplayingMessage(client, nowPlayingMessage);
					}
