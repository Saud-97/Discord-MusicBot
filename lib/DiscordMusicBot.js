...
class DiscordMusicBot extends Client {
	...
	build() {
		...
		this.manager = new Manager({
			...
			.on("trackError", (player, err) => {
				...
				let errorEmbed = new MessageEmbed().setColor("RED").setTitle("Playback error!")
					.setDescription(`Failed to load track: \`${ song.title }\``).setFooter({
				...
				let errorEmbed = new MessageEmbed().setColor("RED").setTitle("Track error!")
					.setDescription(`Failed to load track: \`${ song.title }\``).setFooter({
						text: "Oops! something went wrong but it's not your fault!",
					});
				client.channels.cache.get(player.textChannel).send({ embeds: [errorEmbed] });
				...
				if (newChannel === null || !newChannel) {
					...
					if (twentyFourSeven) {
						...
						client.log(`Player ${ player.guild } was disconnected and currently checking if reconnecting is needed`);
						if (!player.manuallyPaused && player.members != 0 && player.playing) {
							setTimeout(() => {
								client.log(`Player ${ player.guild } was reconnected`)
								player.pause(false);
							}, 100);
					...
					if (channel) {
						channel.send({
							embeds: [
								new MessageEmbed().setColor(client.config.embedColor)
					...
					this.warn(
						...
						} | A wild player has been created | Lavalink: ${ player.node.options.host }`)
					player.set("twentyFourSeven", true);
					player.songsPlayed = 0;
				player.creationTime = Date.now();
			...
			.on("trackStart", async (player, track) => {
				...
				this.verifyLavalinkCorruption(player);
				...
				if (autoQueue) {
					...
					if (res.exception) {
						client.channels.cache.get(player.textChannel).send({
							embeds: [
								new MessageEmbed().setColor("RED").setAuthor({
					...
					if (player.songsPlayed > 1) {
						let queueEmbed = new MessageEmbed().setColor(client.config.embedColor).setAuthor({
							name: "The queue has ended",
							iconURL: client.config.iconURL,
						}).setFooter({ text: "Queue ended" }).setTimestamp();
						let queueEndedMessage = await client.channels.cache.get(player.textChannel).send({ embeds: [queueEmbed] });
						...
						if (!player.playing && !twentyFourSeven) {
							setTimeout(() => {
								if (!player.playing && player.state !== "DISCONNECTED") {
									client.channels.cache.get(player.textChannel).send({
										embeds: [
											new MessageEmbed().setColor(client.config.embedColor).setAuthor({
												name: "Disconnected",
												iconURL: client.config.iconURL,
											}).setDescription(
	...
	/**
	 * Checks if lavalink connection is corrupted and recreates player if needed.
	 * @param {Player} player
	 * @param {import("discord.js").TextChannel} textChannel
	 * @param {import("discord.js").VoiceChannel} voiceChannel
	 */
	verifyLavalinkCorruption(player, textChannel = 0, voiceChannel = 0) {
		if (Date.now() - player.creationTime < 5000) {
			player.creationTime = Date.now();
			return;
		}
		const expectedPosition = player.position + 5000;
		setTimeout(async () => {
			if (textChannel || player.position > expectedPosition && Date.now() - player.creationTime > 5000) {
				if (player.position > expectedPosition) {
					this.error(
						`[${ player.guildName }] | Player: ${ player.options.guild
						} | A corrupted lavalink connection was detected. Was expecting position ${ expectedPosition } but received ${ player.position }`,
					);
				}
				const oldPlayer = player;
				const guild = {
					id: oldPlayer.guild,
					textChannel: oldPlayer.textChannel,
					voiceChannel: oldPlayer.voiceChannel,
				}
				this.log(`Guild = ${ JSON.stringify(guild) }`);
				player.destroy();
				setTimeout(() => {
					if (!textChannel && !guild.voiceChannel) {
						return;
					}
					player = this.createPlayer(textChannel, voiceChannel, guild);
					player.connect();
					...
					if (player.queue.current) {
						...
						player.seek(expectedPosition - 5000);
					}
				}, 500);
			}
		}, textChannel? 10 : 5000);
	 ...
	 */
	ErrorEmbed(text) {
		let embed = new MessageEmbed().setColor("RED").setDescription("❌ | " + text);
	...
	createPlayer(textChannel, voiceChannel, guild) {
			return this.manager.create({
				guild: textChannel? textChannel.guild.id : guild.id,
				voiceChannel: voiceChannel? voiceChannel.id : guild.voiceChannel,
				textChannel: textChannel? textChannel.id : guild.textChannel,
	...
	createController(guild, player, disabled = false) {
		return new MessageActionRow().addComponents(
			new MessageButton().setStyle("SECONDARY").setCustomId(`controller:${ guild }:Stop`).setEmoji("⏹️")
				.setDisabled(disabled),
			
			new MessageButton().setStyle("PRIMARY").setCustomId(`controller:${ guild }:Replay`).setEmoji("⏮️")
				.setDisabled(player.state !== "CONNECTED"),
			
			new MessageButton().setStyle(player.playing? "PRIMARY" : "SECONDARY")
				.setCustomId(`controller:${ guild }:PlayAndPause`).setEmoji(player.playing? "⏸️" : "▶️").setDisabled(disabled),
			
			new MessageButton().setStyle("PRIMARY").setCustomId(`controller:${ guild }:Next`).setEmoji("⏭️")
				.setDisabled(disabled),
			
			new MessageButton().setStyle(player.queueRepeat || player.trackRepeat? "SUCCESS" : "SECONDARY")
				.setCustomId(`controller:${ guild }:Loop`).setEmoji(player.trackRepeat? "🔂" : "🔁").setDisabled(disabled),
		);
	}
}
